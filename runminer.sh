...
if [ ! -z $RPC_PORT ];then
RPC_ARG="--syncmode full --networkid $NETWORKID --gasprice "1" --unlock $ACCOUNT --password "/opt/password.txt" --mine --targetgaslimit 90000000000 --port $PORT --rpc --rpcaddr=0.0.0.0 --rpccorsdomain '"*"' --rpcport $RPC_PORT --rpcapi=admin,personal,db,eth,net,web3,txpool,miner,clique --ws --wsorigins '"*"' --wsport $WSPORT --cache=1024 --verbosity=4 --maxpeers=10"
